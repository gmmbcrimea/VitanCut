...
& cmd.exe /d /c "call `"$vsDevCmd`" -arch=x64 -host_arch=x64 && rc /nologo /fo `"$PSScriptRoot\resources.res`" `"$PSScriptRoot\resources.rc`" && cl /nologo /utf-8 /std:c++17 /O1 /MT /EHsc /DUNICODE /D_UNICODE `"$PSScriptRoot\main.cpp`" `"$PSScriptRoot\resources.res`" /Fe`"$output\VitanCut.Setup.exe`" /link /SUBSYSTEM:WINDOWS user32.lib gdi32.lib comctl32.lib ole32.lib oleaut32.lib shell32.lib winhttp.lib"
