...
create or replace function public.is_vitan_member(p_workspace_id uuid)
...
revoke all on public.workspaces, public.workspace_members, public.workspace_snapshots, public.workspace_snapshot_history from anon, authenticated;
grant select on public.workspaces, public.workspace_members, public.workspace_snapshots, public.workspace_snapshot_history to authenticated;

create policy "members read workspaces" on public.workspaces for select to authenticated
using (public.is_vitan_member(id));
create policy "members read workspace members" on public.workspace_members for select to authenticated
using (public.is_vitan_member(workspace_id));
create policy "members read snapshots" on public.workspace_snapshots for select to authenticated
using (public.is_vitan_member(workspace_id));
create policy "members read snapshot history" on public.workspace_snapshot_history for select to authenticated
using (public.is_vitan_member(workspace_id));

create or replace function public.create_vitan_workspace(p_name text, p_payload jsonb)
